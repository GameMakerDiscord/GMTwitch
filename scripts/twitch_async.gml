    ...
    for(var i=0; i<ds_map_size(global.Update_list); i++;)
        ...
        if (update_id == undefined)
            ...
            if (request_id == update_id)
                ...
                if (global.Twitch_logs)
                    {
                    var file = file_text_open_write(working_directory + "\" +string(channel_id) + "-result.txt");
    ...
    for(var i=0; i<ds_map_size(global.Thumb_list); i++;)
        ...
        if (update_id == undefined)
            ...
            if (request_id == update_id)
                ...
                if (global.Twitch_logs)
                    {
                    var file = file_text_open_write(working_directory + "\" +string(channel_id) + "-thumb-result.txt");
                    file_text_write_string(file,result);
                    file_text_close(file);
                    }
